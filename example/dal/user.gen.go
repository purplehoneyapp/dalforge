...
Version [ba8fbf058d3e7b6a9c33897f269806386819bbd83061ea2b4dea9d616f62291b]
*/
package dal
...
func (d *userRepository) getByID(ctx context.Context, id int64) (*User, error) {
    ...
    query := `
        SELECT id, version, age, birthdate, email, meta, status, uid, created, updated
        FROM users
        WHERE id = ?
    `
...
func (d *userRepository) listById(ctx context.Context, startID int64, pageSize int) ([]*User, error) {
    ...
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users ORDER BY id LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE id > ? ORDER BY id LIMIT ?`
    }
...
func (d *userRepository) listByBday(ctx context.Context, birthdate *time.Time, startID int64, pageSize int) ([]*User, error) {
    ...
    // if startID is zero then query is different for pagination
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE birthdate < ? ORDER BY birthdate DESC, id DESC LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE birthdate < ? AND id < ? ORDER BY birthdate DESC, id DESC LIMIT ?`
    }
...
func (d *userRepository) listByAge(ctx context.Context, minage int8, startID int64, pageSize int) ([]*User, error) {
    ...
    // if startID is zero then query is different for pagination
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE age = ? OR age > ? ORDER BY created, id LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE age = ? OR age > ? AND id > ? ORDER BY created, id LIMIT ?`
    }
...
func (d *userRepository) listByStatus(ctx context.Context, status *string, startID int64, pageSize int) ([]*User, error) {
    ...
    // if startID is zero then query is different for pagination
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE status = ? ORDER BY created, id LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, meta, status, uid, created, updated FROM users WHERE status = ? AND id > ? ORDER BY created, id LIMIT ?`
    }
...
func (d *userRepository) countListByAge(ctx context.Context, minage int8) (int64, error) {
	...
	// if startID is zero then query is different for pagination
	query := `SELECT count(*) FROM users WHERE age = ? OR age > ?`
