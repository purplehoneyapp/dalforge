...
Version [dfa96cae31ff48f4ce003822d63b2565d758e0959ba68ca768898cd29fd8e7c6]
*/
package dal
...
func (d *userRepository) create(ctx context.Context, entity *User) (*User, error) {
	...
	query := `
		INSERT INTO users (age,birthdate,email,status,uid,
created,updated)
		VALUES (?,?,?,?,?,?,?)
	`
	...
	result, err := db.ExecContext(ctx, query,entity.Age, entity.Birthdate, entity.Email, entity.Status, entity.Uid,
...
func (d *userRepository) createBulk(ctx context.Context, entities []*User) ([]*User, error) {
    ...
    for i := 0; i < len(entities); i += batchSize {
        ...
        query := fmt.Sprintf(`
            INSERT INTO users
            (age,birthdate,email,status,uid,
created,updated)
            VALUES %s
...
func (d *userRepository) update(ctx context.Context, entity *User) error {
	...
	query := `
		UPDATE users
		SET age = ?,birthdate = ?,email = ?,status = ?,uid = ?, updated=?, version = version + 1
	...
	res, err := db.ExecContext(ctx, query,entity.Age, entity.Birthdate, entity.Email, entity.Status, entity.Uid,
...
func (d *userRepository) getByID(ctx context.Context, id int64) (*User, error) {
    ...
    query := `
        SELECT id, version, age, birthdate, email, status, uid, created, updated
        FROM users
        WHERE id = ?
    `
    ...
    err := row.Scan(
        &entity.ID,
        &entity.Version,&entity.Age,&entity.Birthdate,&entity.Email,&entity.Status,&entity.Uid,
        &entity.Created,
        &entity.Updated,
    )
    if err != nil {
...
func (d *userRepository) listById(ctx context.Context, startID int64, pageSize int) ([]*User, error) {
    ...
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users ORDER BY id LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE id > ? ORDER BY id LIMIT ?`
    }
...
func (d *userRepository) listByBday(ctx context.Context, birthdate *time.Time, startID int64, pageSize int) ([]*User, error) {
    ...
    // if startID is zero then query is different for pagination
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE birthdate < ? ORDER BY birthdate DESC, id DESC LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE birthdate < ? AND id < ? ORDER BY birthdate DESC, id DESC LIMIT ?`
    }
...
func (d *userRepository) listByAge(ctx context.Context, minage int8, startID int64, pageSize int) ([]*User, error) {
    ...
    // if startID is zero then query is different for pagination
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE age = ? OR age > ? ORDER BY created, id LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE age = ? OR age > ? AND id > ? ORDER BY created, id LIMIT ?`
    }
...
func (d *userRepository) listByStatus(ctx context.Context, status *string, startID int64, pageSize int) ([]*User, error) {
    ...
    // if startID is zero then query is different for pagination
    if startID == 0 {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE status = ? ORDER BY created, id LIMIT ?`
    } else {
        query = `SELECT id, version, age, birthdate, email, status, uid, created, updated FROM users WHERE status = ? AND id > ? ORDER BY created, id LIMIT ?`
    }
